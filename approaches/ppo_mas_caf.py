...
class PPO_MAS_CAF():
    def __init__(self,
        ...
        print ('kld_lambda : ', self.kld_lambda)
    ...
    def update(self, rollouts, task_num):
        ...
        self.actor_critic.base.adaptor = nn.init.normal_(nn.Parameter(torch.ones((8, self.actor_critic.base.nLearner))))
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                values, action_log_probs, dist_entropy, _, value_exp = self.actor_critic.evaluate_actions(
                ...
                kld_loss = self.kld_lambda * self.kld(value_exp)

                reg_loss = self.mas_loss() #self.reg_lambda * self.mas_loss()
                reg_loss += kld_loss
...
#             rollouts.after_update()
    ...
    def mas_loss(self):
        ...
        if self.mas_task_count > 0:
            losses = []
            losses_l2 = []
            # If "offline MAS", loop over all previous tasks (if "online MAS", [mas_task_count]=1 so only 1 iteration)
            for task in range(1, self.mas_task_count + 1):
                ...
                for n, p in self.actor_critic.named_parameters():
                    if p.requires_grad:
                        ...
                        if self.adapt_af:
                            if 'net' in n or 'fc' in n:
                                softmax_adaptor = self.actor_critic.base.nLearner * F.softmax(self.actor_critic.base.adaptor[weight_id % weight_num])
                                losses_l2.append(softmax_adaptor[weight_id // weight_num] * torch.sum(p.pow(2)))
                                weight_id += 1
                            elif 'adaptor' not in n:
                                losses_l2.append((p ** 2).sum())
                        #losses_l2.append((p ** 2).sum())
            return self.reg_lambda * (1. / 2) * sum(losses) + self.af_lambda * (1. / 2) * sum(losses_l2)
        else:
            return 0.
