...
class KLD_adapt(nn.Module):
    def __init__(self):
        super(KLD_adapt, self).__init__()
        self.criterion_KLD = nn.KLDivLoss(reduction='batchmean')

    def forward(self, x, t, model, scale=100):
        KLD_loss = 0
        if t == 0:
            for k in range(len(x)):
                for l in range(len(x)):
            ...
            for k in range(len(x)):
                adaptor_kld_sig = len(x) * F.softmax(scale * model.adaptor_kld[k])
                for l in range(len(x)):
                    if l != k:
                        KLD_loss += adaptor_kld_sig[l] * self.criterion_KLD(F.log_softmax(x[k], dim=1), F.softmax(x[l], dim=1).detach())
        return KLD_loss
