...
if __name__ == "__main__":
    ...
    parser.add_argument('--lamb', default='1', type=float, help='ewc_loss') #('--lamb', type=int, default=50, help='ewc_loss')
    parser.add_argument('--lamb1', default='1', type=float, help='kld_loss')
    parser.add_argument('--lamb2', default='1', type=float, help='l2_loss')
    ...
    parser.add_argument('--adapt_af', action='store_true', help='use task adaptive gate')
    ...
    print("args.lamb", args.lamb)
    print("args.lamb1", args.lamb1)
    print("args.lamb2", args.lamb2)
    print("args.n_memories", args.n_memories)
    ...
    for run in range(args.n_runs):
        ...
        for task , tr_loader in enumerate(train_loader):
            ...
            model.train()
            if 'caf' in args.model:
                model.adapt_af = True #args.adapt_af
            ...
            if task != args.n_tasks-1:

                # save old param
                old_param = {}
                for n, p in model.net.named_parameters():
                    old_param[n] = p.data.clone().detach()

                fisher = model.fisher
                if task > 0:
                    fisher_old = {}
                    for n, _ in model.net.named_parameters():
                ...
                num_sample = 0
                for x, y in tqdm(tr_loader, ncols=69):
                    loss, fisher = model.observe_fisher(x.cuda(), task, y.cuda(), fisher)
                    num_sample += x.size(0)
                model.train()

                with torch.no_grad():
                    for n, _ in model.net.named_parameters():
                        fisher[n] = fisher[n] / num_sample

                if task > 0:
                    for n, _ in model.net.named_parameters():
                        fisher[n] = (fisher[n] + fisher_old[n] * task) / (task + 1)  # Checked: it is better than the other option
                        # self.fisher[n] = self.fisher[n] + fisher_old[n]

                model.fisher = fisher
                model.old_param = old_param
