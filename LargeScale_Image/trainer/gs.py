        ...
        with torch.no_grad():
            for (name,module),(_,module_old) in zip(self.model.named_children(),self.model_fixed.named_children()):
                ...
                if t>0:
                    ...
                    aux = F.threshold(norm - self.omega[key].cuda()*lamb*lr, 0, 0, False)
                    boonmo = lr*lamb*self.omega[key].cuda() + aux
                    alpha = (aux / boonmo)
                    alpha[alpha!=alpha] = 1
                        
                    coeff_alpha = alpha * self.mask[key].cuda()
                    coeff_beta = (1-alpha) * self.mask[key].cuda()
    ...
    def cal_omega(self):
        ...
        for samples in tqdm(self.omega_iterator,desc='Omega update'):
            ...
            for name, param in self.model.named_parameters():
                ...
                name = name.split('.')[:-1]
                name = '.'.join(name)
                param_R[name] += self.model.act[cnt].abs().detach().cpu()*data.shape[0]
                cnt+=1 
