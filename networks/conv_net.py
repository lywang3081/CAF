...
# 6-layer CNN borrowed from AGS-CL
class Net(nn.Module):
    def __init__(self, inputsize, taskcla):
        ...
        self.conv1 = nn.Conv2d(ncha,self.nc,kernel_size=3,padding=1)
        s = compute_conv_output_size(size,3, padding=1) # 32
        self.conv2 = nn.Conv2d(self.nc,self.nc,kernel_size=3,padding=1)
        s = compute_conv_output_size(s,3, padding=1) # 32
        s = s//2 # 16
        self.conv3 = nn.Conv2d(self.nc,self.nc*2,kernel_size=3,padding=1)
        s = compute_conv_output_size(s,3, padding=1) # 16
        self.conv4 = nn.Conv2d(self.nc*2,self.nc*2,kernel_size=3,padding=1)
        s = compute_conv_output_size(s,3, padding=1) # 16
        s = s//2 # 8
        self.conv5 = nn.Conv2d(self.nc*2,self.nc*4,kernel_size=3,padding=1)
        s = compute_conv_output_size(s,3, padding=1) # 8
        self.conv6 = nn.Conv2d(self.nc*4,self.nc*4,kernel_size=3,padding=1)
        s = compute_conv_output_size(s,3, padding=1) # 8
#         self.conv7 = nn.Conv2d(128,128,kernel_size=3,padding=1)
#         s = compute_conv_output_size(s,3, padding=1) # 8
        s = s//2 # 4
        self.fc1 = nn.Linear(s*s*self.nc*4,self.nc*8) # 2048
        ...
        for t,n in self.taskcla:
            self.last.append(torch.nn.Linear(self.nc*8,n))
